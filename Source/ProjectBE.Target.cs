...
public class ProjectBETarget : TargetRules
{
	public ProjectBETarget(TargetInfo Target) : base(Target)
	{
		...
		ExtraModuleNames.AddRange(new string[] { "ProjectBE" });

		ProjectBETarget.ApplySharedBETargetSettings(this);
	}

	private static bool bHasWarnedAboutShared = false;

	internal static void ApplySharedBETargetSettings(TargetRules Target)
	{
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			ProjectBETarget.ConfigureGameFeaturePlugins(Target);
		}
		else
		{
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Logger.LogWarning("ProjectBE_EOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}
	}
